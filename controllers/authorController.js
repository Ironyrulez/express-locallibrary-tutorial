...
exports.author_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        // Find all books written by the author, selecting title and summary
...
exports.author_delete_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { return next(err); }
        if (results.author == null) { // No results, redirect to list of all authors
    ...
    async.parallel({
        ...
        if (results.authors_books.length > 0) {
            ...
            Author.findByIdAndRemove(req.body.authorid,
                function deleteAuthorRedirect(err) {
                    if (err) { return next(err); }
...
exports.author_update_get = function(req, res) {
    ...
    Author.findById(req.params.id)
        .exec(function(err, author) {
            if (err) { return next(err); }
            if (author == null) { // Author not found but it should exist, therefore error
                var err = new Error('Author not found');
                err.status = 404;
                return next(err);
            }
...
exports.author_update_post = [
    ...
    (req, res, next) => {
        ...
        // Create an Author object with sanitized data, keep old id
        var author = new Author({
                first_name: req.body.first_name,
                family_name: req.body.family_name,
                date_of_birth: req.body.date_of_birth,
                date_of_death: req.body.date_of_death,
                _id: req.params.id  // Update this record
            });
